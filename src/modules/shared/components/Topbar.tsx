...
export const TopBar: React.FC<{}> = ({ }) => {
    ...
    //useSelector((state) => undefined);
    const dispatch = useDispatch();
    const notificationIcon =  <svg fill="#ffffff" height="30px" width="30px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
        ...
        <nav>
            {userSession && 
                <ul className='flex items-center justify-center font-semibold'>
                    <li key={`menu`} className='relative group px-3 py-2'>
                      <Badge props={{icon: notificationIcon, bubbleContent: '7', badgeAdditionalClass: '', bubbleAdditionalClass: 'bg-red-500'}} />
                    </li>
                    <li key={`menu_1`} className='relative group px-3 py-2 text-white'>
                        {userSession.user.email}
                    </li>
                    <li key={`menu_2`} className='relative group px-3 py-2 text-white'>
